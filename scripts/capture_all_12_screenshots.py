...
OUTPUT_DIRS = ["playstore_package", "artifacts"]
...
print("Starting 12-screenshot capture sequence...")

# Ensure app is active with 6 items
subprocess.run(["python", "scripts/populate_six_cards.py"], check=True)
time.sleep(1.0)
...
# Scroll inside dialog to show the full icon grid
...
open_item("card-sobriety-100d")
time.sleep(0.4)
# Tap Anchor date (Jun 3, 2026)
tap(700, 1050)
time.sleep(0.5)
capture("screenshot_06_solar_calendar_picker_light.png")
key("KEYCODE_BACK") # Close date picker
key("KEYCODE_BACK") # Close editor
