...
/**
 * Minimalist Compose configuration screen for the Zen Pebble (1x1) Widget.
 * Allows the user to select which specific counter to bind to this 1x1 pebble instance.
 */
class ZenPebbleConfigureActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ZenTheme {
                ...
                ZenPebbleConfigureScreen(
                    items = items,
                    today = today,
                    onSelect = { selectedItem ->
                        val appContext = applicationContext
                        lifecycleScope.launch(Dispatchers.IO) {
                            store.setZenPebbleBinding(appWidgetId, selectedItem.id)
...
@Composable
private fun ZenPebbleConfigureScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.zen_pebble_configure_title).uppercase(),
        ...
        Text(
            text = stringResource(R.string.zen_pebble_configure_subtitle),
            fontSize = 14.sp,
        ...
        if (items.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(
                    items = items,
                    key = { it.id },
                    contentType = { "zen_pebble_configure_item" },
                ) { item ->
                    ItemCard(
                        item = item,
                        onClick = {
