...
/**
 * Minimalist Compose configuration screen for the Zen Horizon Ribbon Widget.
 * Allows the user to select which specific counter to feature on this widget instance.
 */
class ZenHorizonConfigureActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ZenTheme {
                ...
                ZenHorizonConfigureScreen(
                    items = items,
                    today = today,
                    onSelect = { selectedItem ->
                        val appContext = applicationContext
                        lifecycleScope.launch(Dispatchers.IO) {
                            store.setZenHorizonBinding(appWidgetId, selectedItem.id)
...
@Composable
private fun ZenHorizonConfigureScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.zen_horizon_configure_title).uppercase(),
        ...
        Text(
            text = stringResource(R.string.zen_horizon_configure_subtitle),
            fontSize = 14.sp,
        ...
        if (items.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(
                    items = items,
                    key = { it.id },
                    contentType = { "zen_horizon_configure_item" },
                ) { item ->
                    ItemCard(
                        item = item,
                        onClick = {
