...
/**
 * Minimalist Compose configuration screen for the Shuin (金石朱印 · 1x1) Cinnabar Seal Widget.
 * Allows the user to select which specific counter to stamp with this containerless seal.
 */
class ShuinConfigureActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ZenTheme {
                ...
                ShuinConfigureScreen(
                    items = items,
                    today = today,
                    onSelect = { selectedItem ->
                        val appContext = applicationContext
                        lifecycleScope.launch(Dispatchers.IO) {
                            store.setShuinBinding(appWidgetId, selectedItem.id)
...
@Composable
private fun ShuinConfigureScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.shuin_configure_title).uppercase(),
        ...
        Text(
            text = stringResource(R.string.shuin_configure_subtitle),
            fontSize = 14.sp,
        ...
        if (items.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(
                    items = items,
                    key = { it.id },
                    contentType = { "shuin_configure_item" },
                ) { item ->
                    ItemCard(
                        item = item,
                        onClick = {
