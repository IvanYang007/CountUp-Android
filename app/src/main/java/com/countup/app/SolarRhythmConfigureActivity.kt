...
/**
 * Minimalist Compose configuration screen for the Solar Rhythm Widget.
 * Allows the user to select which specific counter to honor on this seasonal canvas instance.
 */
class SolarRhythmConfigureActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ZenTheme {
                ...
                SolarRhythmConfigureScreen(
                    items = items,
                    today = today,
                    onSelect = { selectedItem ->
                        val appContext = applicationContext
                        lifecycleScope.launch(Dispatchers.IO) {
                            store.setSolarRhythmBinding(appWidgetId, selectedItem.id)
...
@Composable
private fun SolarRhythmConfigureScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.solar_rhythm_configure_title).uppercase(),
        ...
        Text(
            text = stringResource(R.string.solar_rhythm_configure_subtitle),
            fontSize = 14.sp,
        ...
        if (items.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(
                    items = items,
                    key = { it.id },
                    contentType = { "solar_rhythm_configure_item" },
                ) { item ->
                    ItemCard(
                        item = item,
                        onClick = {
