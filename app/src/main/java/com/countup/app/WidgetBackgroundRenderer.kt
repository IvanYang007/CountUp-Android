...
object WidgetBackgroundRenderer {
    ...
    fun render(
        ...
        return try {
            ...
            when (active) {
                ...
                BackgroundTheme.AUTO_DAILY -> drawDreamBoatWidget(canvas, w, h)
            }
    ...
    private fun drawDreamBoatWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        // Water Wave Path (low-profile bottom-right sweep)
        ...
        // Compact Mini Boat nestled in far bottom-right (X: 85%, Y: 75%)
    ...
    private fun drawClearSpringWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        val stream = Path().apply {
            moveTo(w * 0.40f, h)
            cubicTo(w * 0.60f, h * 0.88f, w * 0.78f, h * 0.76f, w, h * 0.78f)
        }
        canvas.drawPath(stream, paint)

        // 2 Compact Smooth Moss Stones at far bottom right
        paint.style = Paint.Style.FILL
        paint.color = INK_SAGE
        val stone1 = Path().apply {
            addOval(w * 0.80f, h * 0.80f, w * 0.92f, h * 0.92f, Path.Direction.CW)
        }
        canvas.drawPath(stone1, paint)

        paint.color = INK_OCHRE
        val stone2 = Path().apply {
            addOval(w * 0.70f, h * 0.86f, w * 0.78f, h * 0.94f, Path.Direction.CW)
        }
    ...
    private fun drawDesertSunsetWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        // Crimson Sun setting on right horizon
        paint.color = INK_VERMILION
        paint.style = Paint.Style.FILL
        canvas.drawCircle(w * 0.86f, h * 0.52f, h * 0.22f, paint)

        // Low-profile Golden Sand Dune
        val dune = Path().apply {
            moveTo(w * 0.40f, h)
            cubicTo(w * 0.60f, h * 0.88f, w * 0.78f, h * 0.72f, w, h * 0.70f)
            lineTo(w, h)
            close()
        }
        paint.shader = LinearGradient(
            w * 0.6f, h * 0.7f, w * 0.6f, h,
    ...
    private fun drawEgretsAscendingWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        // Small marsh mound at bottom right
        paint.color = INK_SAGE
        paint.style = Paint.Style.FILL
        val marsh = Path().apply {
            moveTo(w * 0.65f, h)
            cubicTo(w * 0.78f, h * 0.88f, w * 0.88f, h * 0.82f, w, h * 0.80f)
            lineTo(w, h)
            close()
        }
        canvas.drawPath(marsh, paint)

        // 3 Compact Egrets soaring in upper right corner
        paint.style = Paint.Style.STROKE
        ...
        val egrets = listOf(
            Pair(w * 0.74f, h * 0.28f),
            Pair(w * 0.82f, h * 0.36f),
            Pair(w * 0.90f, h * 0.44f),
        )
    ...
    private fun drawPlumShadowWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        // Horizontal Plum Twig entering from right edge
        paint.style = Paint.Style.STROKE
        ...
        val twig = Path().apply {
            moveTo(w, h * 0.35f)
            cubicTo(w * 0.90f, h * 0.40f, w * 0.82f, h * 0.32f, w * 0.76f, h * 0.38f)
        }
        canvas.drawPath(twig, paint)

        val subTwig = Path().apply {
            moveTo(w * 0.86f, h * 0.37f)
            quadTo(w * 0.82f, h * 0.48f, w * 0.78f, h * 0.52f)
        }
        paint.strokeWidth = 1.4f
        canvas.drawPath(subTwig, paint)

        // 3 Cinnabar Plum Petals near right edge
        paint.style = Paint.Style.FILL
        paint.color = INK_VERMILION
    ...
    private fun drawAncientRoadWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        // Low-profile ancient hill ridge
        paint.color = INK_OCHRE
        paint.style = Paint.Style.FILL
        val ridge = Path().apply {
            moveTo(w * 0.50f, h)
            cubicTo(w * 0.68f, h * 0.85f, w * 0.82f, h * 0.74f, w, h * 0.72f)
            lineTo(w, h)
            close()
        }
        canvas.drawPath(ridge, paint)

        // Winding trail path line
        paint.style = Paint.Style.STROKE
        ...
        val path = Path().apply {
            moveTo(w * 0.62f, h)
            quadTo(w * 0.78f, h * 0.86f, w * 0.88f, h * 0.74f)
        }
    ...
    private fun drawSpringRainWidget(canvas: Canvas, w: Float, h: Float) {
        ...
        val hill = Path().apply {
            moveTo(w * 0.55f, h)
            cubicTo(w * 0.72f, h * 0.85f, w * 0.86f, h * 0.78f, w, h * 0.76f)
            lineTo(w, h)
            close()
        }
        canvas.drawPath(hill, paint)

        // Delicate willow branch at top right
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = 1.5f
        paint.strokeCap = Paint.Cap.ROUND
        val willow = Path().apply {
            moveTo(w, h * 0.12f)
            quadTo(w * 0.88f, h * 0.22f, w * 0.82f, h * 0.44f)
        }
